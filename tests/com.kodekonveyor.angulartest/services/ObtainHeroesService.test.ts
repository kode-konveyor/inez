...
import { type Observable, isEmpty } from 'rxjs';
...
describe('Save Hero', () => {
  ...
  test('the api URl is computed from the base URL and the url of the endpoint.', () => {
    ...
    expect(get.mock.calls[0][0]).toBe(
      'BASE_URL' + UrlMapConstants.GET_HEROES_URL
    );
  });

  test('Returns the result from the API', () => {
    expect(sut.run(changeUserAction, ServicesTestData.storeConfigAction)).toBe(
      ret
    );
  });
  ...
  test('Does not call the API if there is no authenticated user', (done) => {
    const result = sut.run(
      changeUser({ payload: null }),
      ServicesTestData.storeConfigAction
    );
