...
export class TransitionTestData {
  ...
  public static initialState_with_heroeditor_changed = produce(initialState, draft => {
    draft.componentstates.heroeditor = this.heroeditor_changed
  })

  public static nonInitialState_with_heroeditor_reset = produce(this.nonInitialState, draft => {
  ...
  public static nonInitialState_authenticated = produce(this.nonInitialState, draft => {
    draft.componentstates.heroes.authenticated = true;
  })

  public static initialState_herofilter_e = produce(this.initialState, draft => {
    draft.componentstates.herofilter.heroFilter = "e";
  })

  public static nonInitialState_herofilter_e = produce(this.nonInitialState, draft => {
    draft.componentstates.herofilter.heroFilter = "e";
  })
  ...
  public static initialState_selected_hero_is_HERO = produce(this.initialState, draft => {
    draft.componentstates.heroeditor = this.heroeditor_with_HERO_selected;
  })

  public static nonInitialState_selected_hero_is_HERO = produce(this.nonInitialState, draft => {
    draft.componentstates.heroeditor = this.heroeditor_with_HERO_selected;
    draft.componentstates.heroitem[HEROITEM_ID_PREFIX + HERO.id.toString()].selected = true
    draft.componentstates.heroitem[HEROITEM_ID_PREFIX + HERO_OTHER.id.toString()].selected = false
  })
